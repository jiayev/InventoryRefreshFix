...
The object-specific collector used when `pendingUpdateObjects` is nonempty also calls the indexed materializer repeatedly. Its menu call site uses a separate bulk session per collection. The replacement preserves the collector's first-contiguous-run rule, including destruction of the first nonmatching row after that run. It appends newly owned descriptors to the caller's array and does not alter the shared collector used by other game systems. See [Native refresh](NativeRefresh.md) for the SE and AE boundaries.
