...
Set `bEnableIncrementalInvalidation=1` in `Data/SKSE/Plugins/InventoryRefreshFix.ini` to enable the incremental UI path. Before rebuilding the native list, the plugin records native form identities, category flags, and Scaleform entry objects. SkyUI-compatible lists recognize objects retained by the game's partial-update path and match newly materialized objects to prior processed entries using the stable form, display text, and filter flag. Entries whose native primitive data is unchanged reuse the processed object; changed and unmatched entries remain raw for normal SkyUI processing. The movie's original `InvalidateListData` callback still runs, preserving filtering, sorting, category, selection, and custom-processor behavior. The vanilla list retains its topology-checked visible-renderer fast path. Missing movie interfaces, unreliable cache state, and unsupported menu layouts automatically use an unmodified full invalidation.
